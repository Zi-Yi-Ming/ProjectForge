...
def test_cli_plan_rule_end_to_end(tmp_path: Path) -> None:
    ...
    base = tmp_path / "base"
    result = runner.invoke(app, ["plan", str(jd_path), "--planner", "rule", "--base-dir", str(base)])
...
def test_cli_plan_writes_task_graph_json(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["plan", str(jd_path), "--planner", "rule", "--base-dir", str(base), "--json-out", str(json_out)],
    )
    assert result.exit_code == 0, result.output + result.stderr
...
def test_cli_plan_llm_without_key_degrades_to_rule(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["plan", str(jd_path), "--planner", "llm", "--base-dir", str(base)],
...
def test_cli_plan_rejects_unknown_planner(tmp_path: Path) -> None:
    jd_path = _jd_file(tmp_path)
    result = runner.invoke(app, ["plan", str(jd_path), "--planner", "magic"])
    assert result.exit_code != 0


def test_cli_plan_missing_file(tmp_path: Path) -> None:
    result = runner.invoke(app, ["plan", str(tmp_path / "nope.md"), "--planner", "rule"])
