...
def test_diamond_dag_parallel_then_join(tmp_path: Path) -> None:
    graph = TaskGraph(
        project="demo",
        tasks=[
            ...
            Task(id="T4", phase_id="P1", title="T4", goal="g", why="w", dependencies=["T2", "T3"], status=TaskStatus.PENDING, acceptance_criteria=[], out_of_scope=[], implementation_scope="core"),
        ],
    )
    outcomes = {"T2": _passed("T2"), "T3": _passed("T3"), "T4": _passed("T4")}
    ...
    run = orchestrator.run(graph, "run-diamond", tmp_path / "runs" / "run-diamond")
    assert run.status.value == "COMPLETED"
    assert set(run.completed_tasks) >= {"T2", "T3", "T4"}
